...
if(isset($_POST['submit'])){
	...
	if(isset($ten_sp) && isset($gia_sp) && isset($khuyen_mai) && isset($trang_thai) && isset($chi_tiet_sp)){
		if(isset($anh_sp)){
		...
		$sql = "UPDATE sanpham SET id_dm = $id_dm, 
			...
			chi_tiet_sp = '$chi_tiet_sp' WHERE id_sp = $id_sp";	
		$query = mysql_query($sql);
...
<div id="main">
    ...
    <table id="add-prd" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <td><label>Giá bán lẻ</label><br /><input type="text" name="gia_sp" value="<?php if(isset($_POST['gia_sp'])){echo $_POST['gia_sp'];}else{echo $row['gia_sp'];}?>" /> VNĐ <?php if(isset($error_gia_sp)){echo $error_gia_sp;}?></td>
        </tr>
        <tr>
            <td><label>Giá T2</label><br /><input type="text" name="gia_sp_3" value="<?php if(isset($_POST['gia_sp_2'])){echo $_POST['gia_sp_2'];}else{echo $row['gia_sp_2'];}?>" /> VNĐ <?php if(isset($error_gia_sp_2)){echo $error_gia_sp_2;}?></td>
        </tr>
        <tr>
            <td><label>Giá T1</label><br /><input type="text" name="gia_sp_2" value="<?php if(isset($_POST['gia_sp_3'])){echo $_POST['gia_sp_3'];}else{echo $row['gia_sp_3'];}?>" /> VNĐ <?php if(isset($error_gia_sp_3)){echo $error_gia_sp_3;}?></td>
        </tr>
        
        <tr>
            <td><label>Giá Net</label><br /><input type="text" name="gia_sp_4" value="<?php if(isset($_POST['gia_sp_4'])){echo $_POST['gia_sp_4'];}else{echo $row['gia_sp_4'];}?>" /> VNĐ <?php if(isset($error_gia_sp_4)){echo $error_gia_sp;}?></td>
        </tr>
        <tr>
            <td><label>Giá Net Net</label><br /><input type="text" name="gia_sp_5" value="<?php if(isset($_POST['gia_sp_5'])){echo $_POST['gia_sp_5'];}else{echo $row['gia_sp_5'];}?>" /> VNĐ <?php if(isset($error_gia_sp_5)){echo $error_gia_sp;}?></td>
        </tr>

       
        <tr>
            <td><label>Tình trạng</label><br />
            	<label><input type="radio" name="trang_thai" <?php if($row['tinh_trang']==1) echo 'checked';?> value="1" /> Còn hàng</label>
            	<label><input type="radio" name="trang_thai" <?php if($row['tinh_trang']==2) echo 'checked';?> value="2" /> Tạm hết </label>
            	<label><input type="radio" name="trang_thai" <?php if($row['tinh_trang']==0) echo 'checked';?> value="0" /> Hết hàng</label>
